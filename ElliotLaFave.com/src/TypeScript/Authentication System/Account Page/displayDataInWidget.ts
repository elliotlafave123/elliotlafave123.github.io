...
export const displayDataInWidget = () => {
  ...
  fullnameContainer.innerHTML = State.fullName;
  emailContainer.innerHTML = State.emailAddress;
  setProfileColor();
};
...
const setProfileColor = () => {
  ...
  if (profileImageLetter) profileImageLetter.innerText = State.fullName.substring(0, 1);
  // Clear classes and add correct color
  profileImage.removeAttribute("class");
  profileImage.classList.add("profileImage");
  switch (State.profileImgColor) {
    case "blue":
